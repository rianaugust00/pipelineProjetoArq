...
instrucoes = 16  # Número máximo de instruções
...
# Função para atualizar o pipeline
def atualizar_pipeline(matriz_labels, intervalo=3):
    ciclo = 0
    while True:
...
def adicionar_instrucao(matriz_labels, intervalo=4):
    linha_instrucoes = 0
...
def iniciar_simulacao():
    ...
    # Iniciar a simulação em threads separadas
    threading.Thread(target=atualizar_pipeline, args=(matriz_labels,)).start()
...
iniciar_button = tk.Button(input_frame, text="Iniciar Simulação", command=iniciar_simulacao)
iniciar_button.grid(row=0, column=0, padx=10, pady=10)
